...
import { getPageTypeFromPath, requiresAuth, ALIAS_ROUTES } from "./routes";

// Mapeo de slugs estáticos a IDs de cursos
const staticSlugToCourseId: Record<string, string> = {
    "DIPLOMADO-CONTRATACION-PUBLICA-BAJO-LEY-32069-SU-REGLAMENTO": "curso-cp",
...
function AppContent() {
    ...
    const [userRole, setUserRole] = useState<
        "student" | "teacher" | "admin" | "superadmin" | "administration"
    >("student");
    ...
    useEffect(() => {
        const protectedPages: Page["type"][] = [
            "dashboard",
            "enrollment",
            "profile",
        ];
            ...
            case "courseDetail": {
                ...
                const relatedCourses = courses
                    ...
                    .sort((a, b) => {
                        ...
                        const aMatchCategory =
                            a.category === course.category ? 1 : 0;
                        const bMatchCategory =
                            b.category === course.category ? 1 : 0;
                        if (aMatchCategory !== bMatchCategory)
                            return bMatchCategory - aMatchCategory;
    ...
    return (
        ...
        >
                ...
                currentPage.type !== "profile" && (
                    <Header
                        currentPage={(() => {
                            ...
                            if (currentPage.type === "courses")
                                return "/courses";
                            if (currentPage.type === "courseDetail")
                                return "/courses";
                            if (currentPage.type === "about")
                                return "/nosotros";
                            if (currentPage.type === "contact")
                                return "/contacto";
                            if (currentPage.type === "forInstitutions")
                                return "/para-instituciones";
                            if (currentPage.type === "complaintChannel")
                                return "/denuncias";
                            return "/";
                        })()}
                    />
            ...
            <main style={{ padding: 0, margin: 0 }}>{renderPage()}</main>
